...
if (isset($_POST['page'])) {
    ...
    if ($_POST['page'] == 'register') {
        ...
        if (isset($_POST['action']) && $_POST['action'] == 'register') {
            ...
            // Email sending disabled — registration will not send verification emails.
            echo json_encode(array('success' => true));
            exit;
        }
    }
    ...
    if ($_POST['page'] == 'login' && $_POST['action'] == 'login') {
        ...
        // Use the result count instead of calling total_row() which would re-execute
        if (count($result) > 0) {
    ...
    /* =========================
       CHANGE PASSWORD
    ========================== */
    if ($_POST['page'] == 'profile' && $_POST['action'] == 'change_password') {
        $output = array();
        ...
        if ($new_password !== $confirm_password) {
            $output['error'] = 'New password and confirm password do not match.';
            echo json_encode($output);
            exit;
        }
        ...
        if (strlen($new_password) < 6) {
            $output['error'] = 'New password must be at least 6 characters long.';
            echo json_encode($output);
            exit;
        }
    
        // Get current password from database
        $exam->data = array(':user_id' => $user_id);
        $exam->query = "SELECT user_password FROM user_table WHERE user_id = :user_id";
        $result = $exam->query_result();
        ...
        if (!password_verify($current_password, $result[0]['user_password'])) {
            $output['error'] = 'Current password is incorrect.';
            echo json_encode($output);
            exit;
        }
        ...
        $exam->query = "UPDATE user_table SET user_password = :user_password WHERE user_id = :user_id";
        ...
        try {
            ...
            $output['error'] = 'Failed to change password. Please try again.';
        }
