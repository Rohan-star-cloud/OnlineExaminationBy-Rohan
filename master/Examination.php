...
class Examination
{
	...
	function __construct()
	{
		...
		// Start session only if not already started to avoid PHP notice
	...
	function execute_query()
	{
		...
		// Try executing with provided data if any. If the statement has no matching
		// placeholders but $this->data is non-empty (leftover), PDO will throw
		// HY093. Catch that and retry execute() without parameters.
		try {
			...
			// If parameter mismatch, retry without params; otherwise rethrow
			if ($e->getCode() === 'HY093' || strpos($e->getMessage(), 'Invalid parameter number') !== false) {
				$this->statement->execute();
			} else {
				throw $e;
			}
