...
class Examination
{
    ...
    function __construct()
    {
        ...
        try {
            $this->connect = new PDO(
                "mysql:host=$this->host;dbname=$this->database;port=3306;",
                $this->username,
                $this->password,
                array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION)
            );
        } catch (PDOException $e) {
            // Attempt fallback to 127.0.0.1 (force TCP)
            if ($this->host === 'localhost') {
                try {
                    ...
                    $entry = "[".date('Y-m-d H:i:s')."] DB connect error: " . $e2->getMessage() . "\n";
                    @file_put_contents($log, $entry, FILE_APPEND);
                    echo '<h2>Database connection failed</h2>';
                    echo '<p>Please ensure MySQL / MariaDB is running (start XAMPP MySQL service) and that your DB settings in <code>master/Examination.php</code> are correct.</p>';
                    echo '<p>An error was logged to <code>db_error.log</code>. Contact the site administrator if this persists.</p>';
                    exit;
                }
            } else {
                // Host isn't localhost and initial connect failed — log and show message.
                $log = __DIR__ . '/../db_error.log';
                $entry = "[".date('Y-m-d H:i:s')."] DB connect error: " . $e->getMessage() . "\n";
                @file_put_contents($log, $entry, FILE_APPEND);
                echo '<h2>Database connection failed</h2>';
                echo '<p>Please ensure MySQL / MariaDB is running and reachable at ' . htmlspecialchars($this->host) . '.</p>';
                echo '<p>An error was logged to <code>db_error.log</code>. Contact the site administrator if this persists.</p>';
                exit;
            }
        }
    ...
    function execute_query()
    {
        ...
        try {
            if (!empty($this->data)) {
                // Normalize parameter keys: PDO expects parameter names without leading colon.
                $params = [];
            ...
            // Don't attempt to execute the statement without parameters on parameter errors.
            // Re-throw the exception so caller can handle or the shutdown logger can capture it.
            throw $e;
        }
